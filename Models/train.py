...
def get_model_checkpoint_callback(dir, fname):
    return ModelCheckpoint(
        monitor='val/agbd_rmse',
        dirpath=dir,
        filename=f'{fname}_best',
        save_top_k=1,
        mode='min',
        save_last=True)
...
def main():
    ...
    if args.arch == 'rf': 
        train_dataset = RF_GEDIDataset(
            data_path=dataset_path['h5'],
            mode="train",
            args=args,
            years=args.years)
        val_dataset = RF_GEDIDataset(
            data_path=dataset_path['h5'],
            mode="val",
            args=args,
            years=args.years)
        test_dataset = RF_GEDIDataset(
            data_path=dataset_path['h5'],
            mode="test",
            args=args,
            years=args.years)
    ...
    if model_name == 'local' :
        ...
        # if on the cluster, model_name is the JOB ID and MODEL ID in
        # the ensemble
        wandb_logger = WandbLogger(
            entity=args.entity,
            project=args.arch,
    ...
    if args.arch in ['fcn', 'unet', 'nico']:
        ...
        wandb.log({
            'ens_val_rmse': model.ens_val_rmse,
            'ens_val_std': model.ens_val_std,
            'ens_test_rmse': model.ens_test_rmse,
            'ens_test_std': model.ens_test_std})
